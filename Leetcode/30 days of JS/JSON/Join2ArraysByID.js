...
//     1 - If a key only exists in one object, that single key-value pair should be included in the object.
//     2 - If a key is included in both objects, the value in the object from arr2 should override the value from arr1.
...
var join = function(arr1, arr2) {
    ...
    for (let i = 0; i < arr2.length; i++) {
        if (result[arr2[i].id]) for (const key in arr2[i]) result[arr2[i].id][key] = arr2[i][key];  
